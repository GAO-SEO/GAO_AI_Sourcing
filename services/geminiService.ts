...
export const generateProductData = async (productUrl: string, companyName: 'GAOTek' | 'GAORFID'): Promise<{ data: ProductData; sources: Source[] }> => {
  const prompt = `
    ...
    Your task is to access the provided product URL, scrape all relevant information, and then transform it into a structured JSON object.
    ...
    4.  **Extract \`prices\`**:
        *   Find the pricing tiers and structure them as an array of objects, each with a "quantity" string (e.g., "1 to 99 pieces") and a "price" string (e.g., "$ 5.00").

    5.  **Generate \`overview\`**:
        *   Write a product overview.
        *   It MUST begin with "${companyName}'s " followed by the newly generated \`productName\`.
        *   Continue with a summary of the product's features and benefits based on the source text.
    ...
    7.  **Extract \`features\`**:
        *   List the key product features as an array of strings.

    8.  **Process \`specifications\`**:
        *   Extract technical specifications into a list of key-value pairs.
